...
// 1 param usage: node gauguin.js [pieces to download]
//   - First parameter is how many of the pieces to download
//   useful for debugging on small datasets
// 2 param usage: node gauguin.js [first piece] [final piece]
...
var artist = 'Paul Gauguin';
var database_url = 'http://www.gauguingallery.com/gauguin_paintings_list.aspx';
...
env(database_url, function (errors, window) {
  ...
  var links = $('table > tbody > tr > td:nth-child(2) > a');
...
function visit_page(page_url, index, total) {
  ...
  env(page_url, function (errors, window) {
    ...
    image.filename = image.filename.replace('___Lightbox', ''); // For some reason this site adds '___Lightbox' to all filenames
